...
Final iteration of this code containing all previous iterations and their fixes
------------------------------------------------------------
*/
...
//BUTTON STABILITY CHECK (DEBOUNCE)

//Read occupancy button twice for stable confirmation
bool firstOccRead = (digitalRead(occupancyButton) == LOW);
delay(20);
bool secondOccRead = (digitalRead(occupancyButton) == LOW);

if (firstOccRead && secondOccRead) {
  occupancyState = true;
} else {
  occupancyState = false;
}

//Read override button twice for stable confirmation
bool firstOverRead = (digitalRead(overrideButton) == LOW);
delay(20);
bool secondOverRead = (digitalRead(overrideButton) == LOW);

if (firstOverRead && secondOverRead) {
  overrideState = true;
} else {
  overrideState = false;
}
  ...
  // =========================================================
           ...
           lightValue > (thresholdValue * 0.65) &&
           occupancyState) {
  ...
  // =========================================================
  else if (lightValue <= (thresholdValue * 0.65) &&
           occupancyState) {
